...
function page({ params }) {
  ...
  async function onSubmit(values) {
    ...
    try {
      ...
      console.log(values);
      let response = await apiClient.post(`/tickets/raise-ticket`, {
        orderId: orderId,
        ...
        <>
          ...
          <div className="bg-white shadow-sm border p-4 rounded-lg mt-6 flex justify-between items-baseline">
            <div>
              ...
              <h1 className="font-semibold text-sm text-neutral-500 my-2">
                Placed On: {formatDate(data?.time)}
              </h1>
              <h1 className="font-semibold text-sm text-neutral-500 my-2">
          ...
          <div className="bg-white shadow-sm border p-2 rounded-lg mt-6 gap-4 grid grid-cols-1 md:grid-cols-3">
            <div className="px-8">
              ...
              <h4 className="text-neutral-500 font-semibold text-sm">
                {data?.address?.flat} {data?.address?.street},{" "}
                {data?.address?.landmark}
              </h4>
              <h4 className="text-neutral-500 font-semibold text-sm">
                {data?.address?.city} {data?.address?.taluqa}{" "}
            ...
            <div className="px-8">
              ...
              <div className="flex justify-between items-center mb-1">
                ...
                <h3 className="flex items-center text-md font-bold">
                  <IndianRupee className="size-4" />
          ...
          <div className="bg-white shadow-sm border p-2 rounded-lg mt-6 gap-4 px-4">
            <h1 className="font-bold text-lg mb-2">
              Item{data?.items?.length > 1 && "s"} in the Order
            </h1>
          ...
          <div className="bg-white shadow-sm border p-2 md:p-4 rounded-lg mt-6 gap-4 grid grid-cols-1 items-center md:grid-cols-3">
            <div className="px-4 md:px-8">
              ...
              <h4 className="text-neutral-500 font-semibold text-sm">
                If you have any issue with the order.
              </h4>
              <h4 className="text-neutral-500 font-semibold text-sm">
                You can raise a request
              </h4>

              <Dialog open={isOpen} onOpenChange={setIsOpen}>
                <DialogTrigger asChild>
                  <Button className=" mt-1 md:mt-4 border-1 shadow w-full bg-white text-black font-bold hover:bg-gray-200">
                    Raise a Request
                  </Button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-sm">
                  <DialogHeader className="mb-4">
                    ...
                    <DialogDescription>
                      Provide details about the issue facing with the order. Our
                      team will contact as soon as possible.
                    </DialogDescription>
                  </DialogHeader>
                  <Form {...form}>
                    <form
                  ...
                  <DialogDescription className="text-xs text-center text-gray-500">
                    You can see all your tickets on the tickets page.
                  </DialogDescription>
                </DialogContent>
              </Dialog>
            </div>
            <div className="px-4 md:px-8">
              ...
              <Button
                className="mt-1 md:mt-4 border-1 w-full shadow bg-white text-black font-bold hover:bg-gray-200"
                onClick={() => {
                  window.open(
